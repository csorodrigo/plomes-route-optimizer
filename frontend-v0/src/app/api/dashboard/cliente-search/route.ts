...
/**
 * Customer search API using cached/synced data from Supabase
 * This API is optimized for production use on Vercel
 * Route: /api/dashboard/cliente-search?query=searchTerm
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🔍 [CLIENTE-SEARCH] Searching for customer: ${query}`);
    ...
    if (!SUPABASE_URL || !SUPABASE_KEY) {
      console.error('❌ [CLIENTE-SEARCH] Missing Supabase configuration');
      return NextResponse.json(
        {
    ...
    console.log('🔧 [CLIENTE-SEARCH] Supabase URL:', SUPABASE_URL ? 'OK' : 'MISSING');
    console.log('🔧 [CLIENTE-SEARCH] Supabase Key:', SUPABASE_KEY ? 'OK' : 'MISSING');
    ...
    if (customerError) {
      console.error('[CLIENTE-SEARCH] Customer search error:', customerError);
      console.error('[CLIENTE-SEARCH] Error details:', JSON.stringify(customerError, null, 2));
    ...
    console.log(`✅ [CLIENTE-SEARCH] Found ${customers.length} matching customers`);

    // Process all customers found
    const customersWithDeals = await Promise.all(customers.map(async (customer: any) => {
      try {
        console.log(`🔍 [CLIENTE-SEARCH] Processing customer: ${customer.name} (ID: ${customer.id})`);
        ...
        if (salesError) {
          console.error('[CLIENTE-SEARCH] Sales error:', salesError);
        ...
        if (customerSalesError) {
          console.log('[CLIENTE-SEARCH] No customer sales summary found');
        }
        ...
        // If no detailed sales but we have summary data, create a synthetic deal
          ...
          console.log(`[CLIENTE-SEARCH] Using summary data for ${customer.name}: ${customerSales.total_deals} deals, R$ ${customerSales.total_revenue}`);
        ...
        console.log(`✅ [CLIENTE-SEARCH] Found ${deals.length} deals for customer ${customer.name}`);
        ...
        console.error(`❌ [CLIENTE-SEARCH] Error processing customer ${customer.name}:`, error);
    ...
    console.error('❌ [CLIENTE-SEARCH] Error:', error);
