...
class PDFExportService {
  ...
  private async addCustomerList(
    ...
    pdf.setTextColor(this.config.colors.primary);
    pdf.setFont('helvetica', 'bold');
    pdf.text('CLIENTES NA ROTA', this.config.margin, startY);
    ...
    // Render customer list in detailed format
    pdf.setFontSize(this.config.bodyFontSize);
    pdf.setTextColor(this.config.colors.text);
    ...
    routeCustomers.forEach((customer, index) => {
      ...
      if (currentY > pageHeight - 60) {
        ...
        pdf.setTextColor(this.config.colors.primary);
        pdf.setFont('helvetica', 'bold');
        pdf.text('CLIENTES NA ROTA', this.config.margin, currentY);
        currentY += 15;
      }
      ...
      pdf.setFontSize(this.config.bodyFontSize);
      pdf.setTextColor(this.config.colors.text);
      pdf.text(`${index + 1}. ${customer.name || 'N/A'}`, this.config.margin, currentY);

      currentY += this.config.lineHeight;

      // Razão Social
      if (customer.razao_social) {
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(this.config.smallFontSize);
        pdf.setTextColor(this.config.colors.secondary);
        const razaoWidth = pdf.getStringUnitWidth(customer.razao_social) * this.config.smallFontSize / pdf.internal.scaleFactor;
        const maxWidth = pageWidth - (2 * this.config.margin);
        const razaoText = razaoWidth > maxWidth
          ? this.truncateText(pdf, customer.razao_social, maxWidth)
          : customer.razao_social;
        pdf.text(`   Razão Social: ${razaoText}`, this.config.margin + 5, currentY);
        currentY += 5;
      }
      ...
      if (customer.cnpj) {
        ...
        pdf.setFontSize(this.config.smallFontSize);
        pdf.setTextColor(this.config.colors.secondary);
        pdf.text(`   CNPJ: ${customer.cnpj}`, this.config.margin + 5, currentY);
        currentY += 5;
      }
      ...
      const addressWidth = pdf.getStringUnitWidth(address) * this.config.smallFontSize / pdf.internal.scaleFactor;
      const maxAddressWidth = pageWidth - (2 * this.config.margin) - 5;
      const addressText = addressWidth > maxAddressWidth
        ? this.truncateText(pdf, address, maxAddressWidth)
        : address;
      pdf.text(`   R ${addressText}`, this.config.margin + 5, currentY);
      currentY += 5;

      // City
      pdf.text(`   ${customer.city || 'N/A'}`, this.config.margin + 5, currentY);
      currentY += 5;

      // Distance
      if (customer.distanceFromPrevious !== undefined) {
        pdf.setTextColor(this.config.colors.accent);
        pdf.text(`   Distância: ${customer.distanceFromPrevious.toFixed(1)} km`, this.config.margin + 5, currentY);
        currentY += 5;
      }
      ...
      pdf.line(this.config.margin, currentY + 2, pageWidth - this.config.margin, currentY + 2);
