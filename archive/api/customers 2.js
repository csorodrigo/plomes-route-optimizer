    ...
    try {
        ...
        try {
            ...
            for (let i = 0; i < maxCustomers; i++) { // FIXED: Process ALL customers!
                ...
                // Format customer data with route-friendly properties
                const customer = {
                    id: contact.Id,
                    ...
                    address: address,
                    full_address: address, // For route optimization compatibility
                    cep: cep,
                    city: city,
                    state: state,
                    latitude: coords ? coords.latitude : null,
                    longitude: coords ? coords.longitude : null,
                    lat: coords ? coords.latitude : null, // Route API compatibility
                    lng: coords ? coords.longitude : null, // Route API compatibility
                    ...
                    geocoding_status: coords ? 'completed' : (address && address.length > 10 ? 'pending' : 'no_address'),
                    route_ready: !!(coords && coords.latitude && coords.longitude),
                    priority: contact.Tags && contact.Tags.some(t => t.TagName && t.TagName.toLowerCase().includes('priorit')) ? 'high' : 'normal'
                };
